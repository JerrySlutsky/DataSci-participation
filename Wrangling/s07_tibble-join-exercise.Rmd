...
You will neDataSci/Classroom` repo:

```
songs <- read_csv("https://github.com/USF-Psych-DataSci/Classroom/raw/master/data/singer/songs.csv")

ed data from Joey Bernhardt's `singer` R package for this exercise. 



You can download the singer data from the `USF-Psych-
...
```
```{r}
full_join(nycflights13::flights, nycflights13::weather, by "year")
```

```{r}
a <-
  full_join(airports, airlines, "name")
```
```{r}
b <-
  full_join(a, flights, "carrier")
```
```{r}
c <- 
  full_join(b, weather, "origin")
```
