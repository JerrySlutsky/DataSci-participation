...
== compares one element at a time
%in% compares all elements at a time
c = concatanate (vector containing these elements)
...
Replace Bangledash with NA_real_

```{r}
gapminder %>% 
  mutate(lifeExp = if_else(country == "Afghanistan",
                           NA_real_,
                           lifeExp))
```
...
gapminder %>% 
mutate(inc_life_exp = lifeExp) %>% # Compute the changes in life expectancy
